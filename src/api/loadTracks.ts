...
import type {
  ApiTrackLoaderContext,
  LoadTracksIdentifierGroups,
  LoadTracksTarget,
...
/**
 * Runtime server shape required by the loadTracks handler.
 */
type LoadTracksRuntime = ApiNodelinkServer & ApiTrackLoaderContext
 ...
 * ```
 */
function normalizeDefaultSearchSource(
...
function parseIdentifier(
  identifier: string,
  defaultSearchSource: string | string[] | undefined
): LoadTracksTarget {
  ...
  const groups = match?.groups as LoadTracksIdentifierGroups | undefined
 ...
 */
function buildWorkerRequest(target: LoadTracksTarget): LoadTracksWorkerRequest {
  if (target.kind === 'url') {
...
async function handler(
  ...
  if (validation !== true) {
    const errorMessage =
      (Array.isArray(validation) ? validation[0]?.message : undefined) ||
  ...
  try {
    ...
    if (sourceWorkerManager) {
      const delegated = sourceWorkerManager.delegate(
        req,
        res,
        workerRequest.task,
