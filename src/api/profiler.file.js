...
async function handler(nodelink, req, res, _sendResponse, parsedUrl) {
  ...
  if (!endpointConfig.allowExternalPatch && !LOOPBACKS.has(remoteAddress)) {
    return sendErrorResponse(
      ...
      403,
      'Forbidden',
      'External access to profiler file endpoint is blocked.',
      parsedUrl.pathname
    )
  }
  ...
  const context = Math.min(
    60,
    Math.max(3, parseLine(parsedUrl.searchParams.get('context'), 8))
  )
  ...
  try {
    ...
    if (!resolvedPath) resolvedPath = absolutePath
