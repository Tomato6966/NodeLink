  ...
  if (req.method === 'PATCH') {
    ...
    if (refreshToken) {
      logger(
        'info',
        'API',
        'Sandboxing new YouTube refresh token for validation.'
      )
      try {
        ...
        logger(
          'info',
          'API',
          'YouTube refresh token validated successfully in sandbox.'
        )
      } catch (error) {
        logger(
          'warn',
          'API',
          `YouTube token validation failed: ${error.message}`
        )
        return sendErrorResponse(
          req,
          res,
    ...
    try {
            ...
            nodelink.workerManager
              ...
              .catch((err) => {
                logger(
                  'error',
                  'API',
                  `Failed to update worker ${worker.id}: ${err.message}`
                )
                return 0
              })
          )
      ...
      logger(
        'error',
        'API',
        `Critical error during config propagation: ${err.message}`
      )
      return sendErrorResponse(
        req,
        res,
