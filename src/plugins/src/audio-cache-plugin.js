...
export default async function audioCachePlugin(nodelink, pluginApi) {
  ...
  const cacheDir = path.resolve(
    process.cwd(),
    configuration.dir || path.join('cache', 'audio')
  )
  const ttlDays = Number.isFinite(configuration.ttlDays)
    ? configuration.ttlDays
    : 7
  const cleanupIntervalHours = Number.isFinite(
  ...
  const allowExceedWhenAllRecent = Boolean(
    configuration.allowExceedWhenAllRecent
  )
  ...
  async function cleanup() {
    ...
    pluginApi.logger(
      'info',
      'Plugin-Cache',
      `Cleanup finished. Removed ${removed} files, freed ${bytesFreed} bytes`
    )
        ...
        if (e.isDirectory()) await walk(p)
          ...
          try {
            ...
            out.push({
              path: p,
              size: st.size || 0,
              atime: st.atimeMs || st.mtimeMs || st.ctimeMs
            })
          } catch {}
        }
      }
  ...
  async function trimToMax(reserveBytes = 0) {
    ...
    if (protectRecentMilliseconds > 0) {
      candidates = candidates.filter(
        (f) => (f.atime || 0) < now - protectRecentMilliseconds
      )
    }
    ...
    if (candidates.length === 0) {
      if (notInUse.length === 0) {
        pluginApi.logger(
          'info',
          'Plugin-Cache',
          `Trim skipped: all files in use; allowing overflow (total=${total}, target=${target})`
        )
      ...
      if (allowExceedWhenAllRecent) {
        pluginApi.logger(
          'info',
          'Plugin-Cache',
          `Trim skipped: all files are recent; allowing overflow (total=${total}, target=${target})`
        )
        return { removed: 0, bytesFreed: 0 }
      }
      files = notInUse
    ...
    pluginApi.logger(
      'info',
      'Plugin-Cache',
      `Trimmed cache: removed ${removed}, freed ${bytesFreed} bytes, total~=${total}`
    )
    return { removed, bytesFreed }
  }
  ...
  pluginApi.addRoute(
    '/v4/cache/stats',
    async (server, req, res, sendResponse) => {
      const s = await getStats()
      const cap = s.maxBytes || null
      const usage = cap
        ? Math.min(100, Math.round((s.bytes / cap) * 100))
        : null
      sendResponse(
        req,
        res,
    ...
    async (server, track, url, protocol, additionalData, next) => {
      ...
      try {
        pluginApi.logger(
          'info',
          'Plugin-Cache',
          `MISS key=${key} -> caching to ${filePath}`
        )
        try {
  ...
  pluginApi.logger(
    'info',
    'Plugin-Cache',
    `audio-cache-plugin initialized at ${cacheDir} (ttlDays=${ttlDays})`
  )
}
