...
export default class BlowfishCBC {
  ...
  private s0: Uint32Array
  private s1: Uint32Array
  private s2: Uint32Array
  private s3: Uint32Array

  private _ivL: number
  private _ivR: number
  private _haveIv: boolean
  ...
  constructor(key: string | Uint8Array) {
    ...
    if (k.length < 72) {
      ...
      for (let i = 0; i < 72; i++) expanded[i] = k[i % k.length] ?? 0
      k = expanded
    }

    for (let n = 0, i = 0; n < 18; n++, i += 4) {
      const w = pack32(k[i] ?? 0, k[i + 1] ?? 0, k[i + 2] ?? 0, k[i + 3] ?? 0)
      this.p[n] = xor(this.p[n] ?? 0, w)
    }
  ...
  setIv(iv: string | Uint8Array | Buffer) {
    ...
    this._ivL = pack32(iv8[0] ?? 0, iv8[1] ?? 0, iv8[2] ?? 0, iv8[3] ?? 0)
    this._ivR = pack32(iv8[4] ?? 0, iv8[5] ?? 0, iv8[6] ?? 0, iv8[7] ?? 0)
    this._haveIv = true
  }
  ...
  private _F(xL: number) {
    ...
    let res = add32(this.s0[a] ?? 0, this.s1[b] ?? 0)
    res = xor(res, this.s2[c] ?? 0)
    res = add32(res, this.s3[d] ?? 0)
    return res
  }
  ...
  private _encryptBlock(xL: number, xR: number): [number, number] {
    ...
    xR = xor(xR, this.p[16] ?? 0)
    xL = xor(xL, this.p[17] ?? 0)
    return [xL, xR]
  }
  ...
  private _decryptBlock(xL: number, xR: number): [number, number] {
    ...
    xR = xor(xR, this.p[1] ?? 0)
    xL = xor(xL, this.p[0] ?? 0)
    return [xL, xR]
  }
