...
export default class BilibiliLyrics {
   ...
   */
  public readonly nodelink: NodelinkInstanceForBilibiliLyrics

  /**
   * Cached WBI mixin key.
   */
  private wbiKeys: string | null

  /**
   * WBI key expiration timestamp in milliseconds.
   */
  private wbiKeysExpiry: number

  /**
   * Creates a new Bilibili lyrics provider.
   * @param nodelink - Runtime NodeLink context.
   */
  public constructor(nodelink: NodelinkInstanceForBilibiliLyrics) {
  ...
  public async getLyrics(
    track: BilibiliLyricsTrackPayload
  ): Promise<BilibiliLyricsResult> {
    const infoCandidate = track.info || track
    const pluginInfo = track.pluginInfo || {}

    const sourceName =
      typeof infoCandidate === 'object' && infoCandidate
        ? (infoCandidate as { sourceName?: string }).sourceName
        : undefined
    const identifier =
      typeof infoCandidate === 'object' && infoCandidate
        ? (infoCandidate as { identifier?: string }).identifier
        : undefined
    ...
    try {
      ...
      if (!aid || !cid) {
        if (bvid.includes('?p=')) bvid = bvid.split('?p=')[0] || bvid
        ...
        const videoBody = body as BilibiliVideoInfoResponse | undefined
        if (videoBody?.code === 0) {
      ...
      const { body: subData } = await makeRequest(
        subUrl.startsWith('//') ? `https:${subUrl}` : subUrl,
        { method: 'GET' }
      )

      const parsedSubData = subData as BilibiliSubtitleFileResponse | undefined
      if (!Array.isArray(parsedSubData?.body)) {
        return { loadType: 'empty', data: {} }
      }

      const lines: LyricsLine[] = parsedSubData.body.map((line) => ({
      ...
      logger(
        'error',
        'Lyrics',
        `Bilibili lyrics failed: ${e instanceof Error ? e.message : String(e)}`
      )
      return { loadType: 'empty', data: {} }
    }
  }
