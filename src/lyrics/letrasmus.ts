 ...
 * @internal
 */
const parseJsonp = (body?: string): LetrasSolrResponse | null => {
  if (!body) return null
  ...
  if (start !== -1 && end > start) {
    return JSON.parse(trimmed.slice(start + 1, end)) as LetrasSolrResponse
  }
  return JSON.parse(trimmed) as LetrasSolrResponse
}
 ...
 * @internal
 */
const extractOmqLyric = (html: string): LetrasOmqLyricPayload | null => {
  const match = html.match(/_omq\.push\(\['ui\/lyric',\s*({[\s\S]*?})\s*,/i)
  if (!match) return null
  try {
    return JSON.parse(match[1] || '{}') as LetrasOmqLyricPayload
  } catch {
    return null
  }
}
...
const extractTranslationLanguages = (
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? (parsed as LetrasTranslationLanguageEntry[])
      : []
  } catch {
    return []
  }
 ...
 * @internal
 */
const parseSubtitle = (subtitle: string): LyricsLine[] => {
  let parsed: unknown
  try {
  ...
  return parsed
    .map((entry) => {
      ...
      const tuple = entry as LetrasSubtitleRawEntry
...
export default class LetrasMusLyrics {
  ...
  public async getLyrics(
    ...
    try {
      ...
      if (!pageUrl) return { loadType: 'empty', data: {} }
      ...
      if (requestedLang) {
        ...
        if (!translationUrl) return { loadType: 'empty', data: {} }

        const translationHtml = await this._fetchHtml(translationUrl)
        if (!translationHtml) return { loadType: 'empty', data: {} }
