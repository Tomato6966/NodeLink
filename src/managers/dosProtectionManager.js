...
export default class DosProtectionManager {
  constructor(nodelink) {
    ...
    this.cleanupInterval = setInterval(() => this._cleanup(), this.config.thresholds.timeWindowMs)
  }

  _cleanup() {
    const now = Date.now()
    for (const [ip, data] of this.ipRequestCounts.entries()) {
      if (now > data.blockedUntil && now - data.lastReset > this.config.thresholds.timeWindowMs) {
  ...
  check(req) {
    ...
    if (!this.ipRequestCounts.has(remoteAddress)) {
      this.ipRequestCounts.set(remoteAddress, { count: 0, lastReset: now, blockedUntil: 0 })
    }
    ...
    if (ipData.count > this.config.thresholds.burstRequests) {
      ...
      logger('warn', 'DosProtection', `IP ${remoteAddress} exceeded burst limit. Blocking for ${this.config.mitigation.blockDurationMs}ms.`)
      return { allowed: false, status: 403, message: 'Forbidden' }
    }
