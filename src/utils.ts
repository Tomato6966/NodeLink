 ...
 * @internal
 */
const runtime = globalThis as typeof globalThis & { nodelink?: NodelinkRuntime }
...
function cleanOldLogs(): void {
  ...
  try {
    ...
    console.error(
      `[${new Date().toISOString().slice(11, 23)}] \x1b[1m\x1b[3;41m[ERROR] >\x1b[0m: Logs > Failed to clean old logs: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
 ...
 * @public
 */
function logger(level: string, ...args: unknown[]): void {
  const effectiveLevel =
    level === 'sources' || level === 'started' || level === 'network'
      ? 'info'
      : level
  const levelIndex = logLevels[effectiveLevel as LogLevelName]
  ...
  if (level === 'debug') {
    ...
    const categoryEnabled =
      debugConfig[category as string] ??
      (categoryKey ? debugConfig[categoryKey as string] : undefined)
 ...
 * @public
 */
const verifyDiscordID = (id: string | number): boolean =>
  DISCORD_ID_REGEX.test(String(id))
 ...
 * @internal
 */
function modifyPayload(nodelink: NodelinkRuntime, data: unknown): unknown {
  ...
  if ('info' in modifiedData && modifiedData.encoded !== undefined) {
    for (const modifier of modifiers) {
      try {
        modifier(modifiedData as never)
      } catch (e) {
        logger(
          'error',
          'PluginManager',
          `Track modifier error: ${e instanceof Error ? e.message : String(e)}`
        )
      }
    }
  }
...
function sendResponse(
  ...
  if (
    ...
    'trace' in finalData &&
    (finalData as { trace?: unknown }).trace &&
    !trace
  ) {
    const { trace: _trace, ...rest } = finalData as Record<string, unknown>
    finalData = rest
  }
...
function getGitInfo(): GitInfo {
  const buildInfo =
    typeof __BUILD_GIT_INFO__ !== 'undefined' ? __BUILD_GIT_INFO__ : undefined
  if (buildInfo) {
    return buildInfo
  }
 ...
 * @public
 */
function getStats(nodelink: NodelinkRuntime): ServerStatsPayload {
  let players = 0
  ...
  let frameStats: ServerStatsPayload['frameStats'] = null
  if (players > 0) {
    ...
    if (nodelink.workerManager) {
      ...
      for (const session of nodelink.sessions.values()) {
        for (const player of session.players.players.values()) {
          ...
          const stats = (
            player.connection as unknown as {
...
function verifyMethod(
  ...
  req: ApiRequest,
  res: ApiResponse,
  expected: ApiHttpMethod | ApiHttpMethod[] | string | string[],
 ...
 * @public
 */
function decodeTrack(encoded: string): EncodedTrackPayload {
  ...
  const readModifiedUTF8From = (
    ...
    while (i < end) {
      const c = buf[i]
      if (c === undefined) throw new Error('Malformed utf')
      ...
      if ((c & 0xe0) === 0xc0) {
        ...
        const c2 = buf[i + 1]
        if (c2 === undefined) throw new Error('Malformed utf')
  ...
  const decodeDetailsAsList = (detailsBuf: Buffer): Array<string | null> => {
    let p = 0
    const ensure2 = (n: number): void => {
  ...
  const tryParseLegacySeekableTrailer = (buf: Buffer): boolean => {
    let p = 0
    try {
      ...
      if (pRef.value !== buf.length) return false
  ...
  try {
    ...
    throw new Error(
      `Decode Error at [${step}]: ${err instanceof Error ? err.message : String(err)} (Buffer pos: ${position}/${buffer.length})`
    )
  }
}
 ...
 * @public
 */
function encodeTrack(track: TrackEncodeInput): string {
  if (!track || typeof track !== 'object') {
    throw new Error('Encode Error: Input track must be a valid object')
  }

  const encodeModifiedUTF8 = (value: string): Buffer => {
  ...
  const writeByte = (v: number): void => push(Buffer.from([v & 0xff]))
  const writeLong = (v: number): void => {
    const b = Buffer.alloc(8)
    b.writeBigInt64BE(BigInt(v))
    push(b)
  }
  const writeUTF = (v: string): void => push(encodeModifiedUTF8(v))
  const writeNullableText = (v: string | null | undefined): void => {
 ...
 * @public
 */
function parseClient(agent?: string | null): ClientInfo | null {
...
async function _internalHttp1Request(
  ...
  const actualLocalAddress =
    localAddress ?? runtime.nodelink?.routePlanner?.getIP?.() ?? undefined
    ...
    const req = lib.request(reqOptions, (res) => {
      ...
      if (
        REDIRECT_STATUS_CODES.includes(
          responseStatus as (typeof REDIRECT_STATUS_CODES)[number]
        ) &&
        location
      ) {
        res.resume()
        ...
        if (setCookie) {
          ...
          const existingCookie = String(
            nextHeaders.cookie || nextHeaders.Cookie || ''
          )
 ...
 * @public
 */
async function http1makeRequest(
  urlString: string,
  options: HttpRequestOptions = {}
): Promise<HttpRequestResult> {
    ...
    try {
      ...
      if (isRetryable && attempt < maxRetries) {
        ...
        logger(
          'warn',
          'Network',
          `Request for ${urlString} failed with ${code}. Retrying in ${delay}ms... (Attempt ${attempt}/${maxRetries})`
        )
...
async function makeRequest(
  ...
  if (loggingConfig.debug?.network) {
    ...
    logger(
      'debug',
      'Network',
      `[${logId}] Headers: ${JSON.stringify(customHeaders, (key, value) =>
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      session = http2.connect(currentUrl.origin, {
        localAddress
      } as http2.ClientSessionOptions)

      const closeSessionGracefully = (): void => {
        if (
          session &&
          !session.closed &&
          !session.destroyed &&
      ...
      req.on('response', async (headers) => {
        ...
        const rawStatus =
          typeof statusHeader === 'number' ? statusHeader : Number(statusHeader)
        ...
        if (
          REDIRECT_STATUS_CODES.includes(
            (statusCode ?? 0) as (typeof REDIRECT_STATUS_CODES)[number]
          ) &&
          location
        ) {
...
function cleanupHttpAgents(): void {
  try {
    ...
    logger(
      'error',
      'Utils',
      `Error cleaning up HTTP agents: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
function applyEnvOverrides(
  ...
  for (const key in config) {
    ...
    if (envValue !== undefined) {
      ...
      applyEnvOverrides(currentValue as Record<string, unknown>, envVarName)
    }
  }
}
