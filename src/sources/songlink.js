...
export default class SongLinkSource {
      ...
      for (const platform of platforms) {
        ...
        try {
          ...
          logger(
            'debug',
            'SongLink',
            `Failed to resolve ${platform} link: ${e.message}`
          )
        }
      }
  ...
  async search(query, _sourceTerm, _searchType = 'track') {
    try {
      ...
      for (const item of results) {
        ...
        const uri =
          (isPodcastEpisode ? episodeUrl : null) ||
  ...
  async _fetchSongLinkData(url) {
    ...
    if (this.useApi) {
      try {
        ...
        if (this.songIfSingle) apiUrl.searchParams.set('songIfSingle', 'true')
        if (this.apiKey) apiUrl.searchParams.set('key', this.apiKey)
  ...
  async _fetchFromHtml(url) {
    try {
      ...
      if (!payload?.linksByPlatform) return null
  ...
  _buildPlatformOrder(linksByPlatform) {
    ...
    const seen = new Set()
    const base =
      this.preferredPlatforms && this.preferredPlatforms.length > 0
