...
async function manageHlsStream(initialUrl, outputStream) {
  ...
  const playlistFetcher = async () => {
    while (!stop) {
      try {
        const { body: playlistContent, error, statusCode } = await http1makeRequest(playlistUrl);
        ...
        if (targetDurationLine) targetDuration = parseInt(targetDurationLine.split(':')[1], 10);
  ...
  const segmentDownloader = async () => {
    while (!stop || segmentQueue.length > 0) {
      ...
      try {
        const { stream: segmentStream, error, statusCode } = await http1makeRequest(segmentUrl, { streamOnly: true });
        if (error || statusCode !== 200) {
          logger('warn', 'Twitch-HLS-Downloader', `Failed segment ${segmentUrl}: ${statusCode}`);
          continue;
        }
        ...
        logger('error', 'Twitch-HLS-Downloader', `Error processing segment ${segmentUrl}: ${e.message}`);
      }
    }
...
export default class TwitchSource {
  ...
  async setup() {
    try {
      ...
      if (setCookieHeader) {
        const deviceIdCookie = Array.isArray(setCookieHeader) ? setCookieHeader.find(c => c.includes('unique_id=')) : setCookieHeader;
        if (deviceIdCookie) {
  ...
  async _loadClip(slug, originalUrl) {
    try {
      ...
      if (!clipData) {
        return { loadType: 'error', data: { message: 'Clip not found', severity: 'common' } };
      }
  ...
  async _loadVod(vodId, originalUrl) {
    try {
      ...
      if (!vodData) {
        return { loadType: 'error', data: { message: 'VOD not found', severity: 'common' } };
      }
  ...
  async _getClipStreamUrl(slug) {
    try {
      ...
      for (const quality of clipData.videoQualities) {
        if (!bestQuality || parseInt(quality.quality) > parseInt(bestQuality.quality)) {
          bestQuality = quality;
        }
      }
  ...
  async _getVodStreamUrl(vodId) {
    try {
      ...
      return { url: vodUrl, protocol: 'hls',
        format: 'mpegts'
      };
    } catch (e) {
      return { exception: { message: e.message, severity: 'fault' } };
  ...
  async loadStream(track, url, protocol) {
    ...
    const { stream, error, statusCode } = await http1makeRequest(url, { streamOnly: true });
    if (error || statusCode !== 200) {
      return { exception: { message: `Failed to load stream: ${error?.message || statusCode}`, severity: 'fault' } };
    }
