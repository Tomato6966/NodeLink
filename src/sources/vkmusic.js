...
async function manageVkHlsStream(url, outputStream, cookie, localAddress) {
  ...
  try {
    const {
      ...
      error,
      statusCode
    } = await http1makeRequest(url, { headers, localAddress })
    if (error || statusCode !== 200)
      throw new Error(`Failed to fetch manifest: ${statusCode}`)
        ...
        if (method === 'NONE') {
          ...
          if (uriMatch) {
            ...
            if (!keyMap.has(keyUrl)) {
              const { body: keyData, error: keyError } = await http1makeRequest(
                keyUrl,
                {
                  headers,
                  localAddress,
    ...
    for (const segment of segments) {
      ...
      try {
        const { body: encryptedData, error: segError } = await http1makeRequest(
          segment.url,
          {
            headers,
            localAddress,
        ...
        if (
          ...
          try {
            ...
            logger(
              'error',
              'VKMusic-HLS',
              `Decryption failed for segment ${segment.sequence}: ${decErr.message}`
            )
            continue
          }
        }
        ...
        logger(
          'error',
          'VKMusic-HLS',
          `Segment processing error (Seq ${segment.sequence}): ${e.message}`
        )
      }
    }
...
export default class VKMusicSource {
      ...
      try {
        ...
        if (response?.[0]) {
          ...
          logger(
            'info',
            'VKMusic',
            `Loaded VKMusic source. Logged in as: ${response[0].first_name} ${response[0].last_name} (${this.userId})`
          )
          return true
        }
      } catch (e) {
        logger(
          'warn',
          'VKMusic',
          `Auth failed: ${e.message}. Falling back to scraping mode.`
        )
      }
    } else {
  ...
  async _refreshAccessToken() {
    ...
    if (error || statusCode !== 200)
      throw new Error(
        `Failed to refresh token: ${error?.message || statusCode}`
      )
    if (body.type === 'okay' && body.data?.access_token) {
      ...
      this.nodelink.credentialManager.set(
        'vk_access_token',
        this.accessToken,
        body.data.expires * 1000 - Date.now()
      )
  ...
  async search(query, sourceTerm) {
    ...
    if (!this.hasToken)
      return {
        exception: {
          message: 'VKMusic search requires valid auth.',
          severity: 'common'
        }
      }
    try {
  ...
  async getRecommendations(query) {
    if (!this.hasToken)
      return {
        exception: {
          message: 'VKMusic recommendations require valid auth.',
          severity: 'common'
        }
      }
    ...
    if (this.hasToken) {
      try {
        ...
        logger(
          'debug',
          'VKMusic',
          `API playlist resolution failed: ${e.message}. Falling back to scraping.`
        )
      }
    }
    return this._scrapePlaylist(url)
  ...
  async _resolveTrack(url) {
    if (this.hasToken) {
      try {
        ...
        if (res?.[0])
          return { loadType: 'track', data: this.buildTrack(res[0]) }
      } catch (e) {
        logger(
          'debug',
          'VKMusic',
  ...
  buildTrack(item) {
    ...
    const trackInfo = {
      ...
      uri: `https://vk.com/audio${id}`,
      artworkUrl:
        item.album?.thumb?.photo_600 || item.album?.thumb?.photo_300 || null,
      isrc: null,
      sourceName: 'vkmusic'
    }
    return {
      encoded: encodeTrack(trackInfo),
      info: trackInfo,
      pluginInfo: { access_key: item.access_key, streamUrl: uri }
    }
  }
  ...
  async getTrackUrl(decodedTrack) {
    ...
    if (!url && this.hasToken) {
      try {
        ...
        if (decodedTrack.pluginInfo?.access_key)
          audios += `_${decodedTrack.pluginInfo.access_key}`
        const res = await this._apiRequest('audio.getById', { audios })
        if (res && res.length > 0) {
          url = res[0].url
    ...
    if (!url) {
      const scrapeRes = await this._scrapeTrack(
        `https://vk.com/audio${decodedTrack.identifier}`
      )
      if (
        scrapeRes.loadType === 'track' &&
        scrapeRes.data.pluginInfo?.streamUrl
      )
    ...
    return {
      exception: { message: 'Failed to retrieve track URL.', severity: 'fault' }
    }
  }
  ...
  async _apiRequest(method, params) {
    ...
    if (error || statusCode !== 200) {
      if (
        (statusCode === 401 || (body && body.error?.error_code === 5)) &&
        this.cookie
      ) {
        await this._refreshAccessToken()
    ...
    if (body.error) {
      ...
      throw new Error(
        `VK API Error ${body.error.error_code}: ${body.error.error_msg}`
      )
    }
  ...
  _unmask_url(mask_url, vk_id) {
    ...
    try {
      const parts = mask_url.split('?extra=')[1].split('#'),
        extra0 = parts[0],
        extra1 = parts[1]
      const split1 = this._b64_decode(extra1).split(String.fromCharCode(11)),
        base = split1[1]
      const maskUrlArr = this._b64_decode(extra0).split('')
      const urlLen = maskUrlArr.length,
        indexes = new Array(urlLen)
