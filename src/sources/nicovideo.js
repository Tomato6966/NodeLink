...
async function manageEncryptedHls(url, stream, headers) {
  try {
    const { body: playlistContent, error, statusCode } = await http1makeRequest(url, { headers });
    ...
    const { body: key, error: keyError, statusCode: keyStatus } = await http1makeRequest(keyUrl, { headers, responseType: 'buffer' });
    if (keyError || keyStatus !== 200) {
      throw new Error(`Failed to fetch decryption key: ${keyError?.message || keyStatus}`);
    }

    const mediaSequenceTag = lines.find(l => l.startsWith('#EXT-X-MEDIA-SEQUENCE:'));
    const mediaSequence = mediaSequenceTag ? parseInt(mediaSequenceTag.split(':')[1], 10) : 0;
    ...
    if (mapTag) {
      ...
      if (mapUriMatch) {
        ...
        const { body: initSegment, error: mapError, statusCode: mapStatus } = await http1makeRequest(mapUrl, { headers, responseType: 'buffer' });
        if (!mapError && mapStatus === 200 && initSegment.length > 0) {
          let decryptedInit = initSegment;
          if (initSegment.length % 16 === 0 && iv) {
            const decipher = crypto.createDecipheriv('aes-128-cbc', key, iv);
            decipher.setAutoPadding(false);
            decryptedInit = Buffer.concat([decipher.update(initSegment), decipher.final()]);
          }
    ...
    logger('debug', 'NicoVideo-HLS', `Found ${segments.length} segments to process`);
    (async () => {
      for (let segIndex = 0; segIndex < segments.length; segIndex++) {
        ...
        try {
          const { body: encryptedSegment, error: segError, statusCode: segStatus } = await http1makeRequest(segmentUrl, { headers, responseType: 'buffer' });
          if (segError || segStatus !== 200) {
            logger('warn', 'NicoVideo-HLS', `Skipping segment ${segIndex + 1}: ${segError?.message || segStatus}`);
            continue;
          }
          const decipher = crypto.createDecipheriv('aes-128-cbc', key, segmentIv);
          decipher.setAutoPadding(false);
          const decryptedSegment = Buffer.concat([decipher.update(encryptedSegment), decipher.final()]);
          if (!stream.destroyed) {
            stream.write(decryptedSegment);
          }
        } catch (decryptError) {
          logger('warn', 'NicoVideo-HLS', `Failed to decrypt segment ${segIndex + 1}: ${decryptError.message}`);
          continue;
        }
      }
      if (!stream.destroyed) {
...
export default class NicoVideoSource {
  ...
  async search(query) {
    ...
    if (error || statusCode !== 200) {
      return { loadType: 'error', data: { message: `Failed to search: ${error?.message || statusCode}`, severity: 'fault' } };
    }
  ...
  async resolve(url) {
    const videoId = url.match(this.patterns[0])?.[1] || url.match(this.patterns[1])?.[1];
    if (!videoId) return { loadType: 'empty', data: {} };
    const { body, error, statusCode } = await http1makeRequest(`https://www.nicovideo.jp/watch/${videoId}?responseType=json`, { headers: this._buildHeaders() });
    if (error || statusCode !== 200) {
      return { loadType: 'error', data: { message: `Failed to resolve URL: ${error?.message || statusCode}`, severity: 'fault' } };
    }
    const jsonLd = body?.data?.metadata?.jsonLds?.find(x => x['@type'] === 'VideoObject');
    const videoIdFromApi = body?.data?.response?.client?.watchId;
    if (!jsonLd || !videoIdFromApi) {
      return { loadType: 'error', data: { message: 'Could not extract video information.', severity: 'common' } };
    }
    const durationStr = jsonLd.duration;
    const durationMs = durationStr ? (parseInt(durationStr.match(/(\d+)S/)?.[1] || 0, 10) * 1000) : 0;
    const track = {
    ...
    return { loadType: 'track', data: { encoded: encodeTrack(track), info: track, pluginInfo: {} } };
  }
  ...
  async getTrackUrl(track) {
    const { body: pageData, error, statusCode } = await http1makeRequest(`${track.uri}?responseType=json`, { headers: this._buildHeaders() });
    if (error || statusCode !== 200) {
      return { exception: { message: `Failed to get track page: ${error?.message || statusCode}`, severity: 'fault' } };
    }
    const response = pageData?.data?.response;
    if (!response) {
      return { exception: { message: 'Failed to extract response data from page', severity: 'fault' } };
    }
    ...
    if (!dmcMedia || !watchTrackId || !accessRightKey) {
      return { exception: { message: 'Failed to extract required DMC info for stream access', severity: 'fault' } };
    }
    ...
    if (streamError || streamStatus !== 201) {
      return { exception: { message: `Failed to get stream access rights: ${streamError?.message || streamStatus}`, severity: 'fault' } };
    }
    const cookie = streamHeaders['set-cookie'] ? (Array.isArray(streamHeaders['set-cookie']) ? streamHeaders['set-cookie'].join('; ') : streamHeaders['set-cookie']) : null;
    const masterPlaylistUrl = streamData.data.contentUrl;
    const { body: masterPlaylistContent, error: masterError, statusCode: masterStatus } = await http1makeRequest(masterPlaylistUrl, { headers: { 'Cookie': cookie } });
    if (masterError || masterStatus !== 200) {
      return { exception: { message: `Failed to fetch master HLS playlist: ${masterError?.message || masterStatus}`, severity: 'fault' } };
    }
    const lines = masterPlaylistContent.split('\n');
    const audioTag = lines.find(l => l.startsWith('#EXT-X-MEDIA') && l.includes('TYPE=AUDIO'));
    if (!audioTag) {
      return {
    ...
    if (!audioUri) {
      return { exception: { message: 'Could not parse audio URI from master playlist', severity: 'fault' } };
    }
  ...
  async loadStream(track, url, protocol, additionalData) {
    if (protocol === 'hls') {
      ...
      const headers = additionalData?.cookie ? { 'Cookie': additionalData.cookie } : {};
      manageEncryptedHls(url, stream, headers);
      return { stream, type: 'fmp4' };
    }
    return { exception: { message: 'Unsupported protocol', severity: 'common' } };
  }
}