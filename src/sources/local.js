...
function detectTypeByMagic(header) {
  ...
  if (header.subarray(0, 3).toString('ascii') === 'FLV') return 'flv'
...
function detectLocalAudioType(filePath, ext = '') {
  try {
    ...
    logger(
      'warn',
      'Sources',
      `Could not read magic bytes for "${filePath}": ${err.message}`
    )
  }
...
export default class LocalSource {
  ...
  getTrackUrl(track) {
    ...
    const streamType =
      track?.pluginInfo?.streamType || detectLocalAudioType(track.uri, ext)
    return {
      url: track.uri,
      protocol: 'local',
      format: streamType,
  ...
  async loadStream(decoded, _url, _protocol, additional) {
    ...
    const streamType =
      decoded?.pluginInfo?.streamType || detectLocalAudioType(decoded.uri, ext)
