...
const createInstance = (rate, channels, app, cached = null) => {
  ...
  const a = name === 'opusscript' && typeof app === 'string' ? Encoder.Application[app] : app;
...
export class Encoder extends Transform {
  ...
  setBitrate(v) {
    ctl(this.enc, OPUS_CTL.BITRATE, Math.min(128000, Math.max(16000, v)), this.lib.name);
  }
  ...
  setPLP(v) {
    ctl(this.enc, OPUS_CTL.PLP, Math.min(100, Math.max(0, v * 100)), this.lib.name);
  }
}
