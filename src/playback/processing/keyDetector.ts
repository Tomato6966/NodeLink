...
const CAMELOT_MAJOR_NUM = [8, 3, 10, 5, 12, 7, 2, 9, 4, 11, 6, 1] as const
// Minor: C=5A, C#=12A, D=7A, D#=2A, E=9A, F=4A, F#=11A, G=6A, G#=1A, A=8A, A#=3A, B=10A
const CAMELOT_MINOR_NUM = [5, 12, 7, 2, 9, 4, 11, 6, 1, 8, 3, 10] as const
...
export interface KeyResult {
  ...
  /** Major or minor mode. */
  mode: 'major' | 'minor'
  /** Camelot wheel notation, e.g. "8B" or "5A". */
  camelot: string
  /** Camelot wheel number 1–12. */
  camelotNum: number
  /** Detection confidence 0–1 (higher = more certain). */
  confidence: number
  /**
   * Temporal key consistency across analysed segments (0–1).
   * Higher values mean the key stayed stable over time.
   */
  stability?: number
  /**
   * Tonal focus score from chroma entropy/triad dominance (0–1).
   * Higher values usually indicate clearer harmonic content.
   */
  tonalClarity?: number
  /**
   * Ambiguity between major/minor interpretation (0–1).
   * Lower values mean the mode decision is more definitive.
   */
 ...
 */
export function estimateKeyFromPcm(
  pcm: Buffer,
  sampleRate: number,
  channels: number
): KeyResult | null {
  ...
  if (totalSamples < sampleRate * 3) return null
  ...
  let globalWeight = 0
  const segmentVotes: Array<{ encoded: number; weight: number }> = []
  ...
  for (let start = 0; start + WINDOW_SIZE <= maxSamples; start += HOP_SIZE) {
    ...
    for (let i = 0; i < WINDOW_SIZE; i++) {
      ...
      sumSq += sample * sample
      mono[i] = sample * (hann[i] as number)
    }

    const rms = Math.sqrt(sumSq / WINDOW_SIZE)
    if (rms < NOISE_FLOOR) continue // Skip silence/noise
...
export function harmonicCompatibilityScore(
  ...
  const stabilityScore = Math.sqrt(
    clamp01(keyA.stability ?? keyA.confidence) *
      clamp01(keyB.stability ?? keyB.confidence)
  )
  const clarityScore = Math.sqrt(
    clamp01(keyA.tonalClarity ?? keyA.confidence) *
...
function findBestKey(
  chroma: Float64Array,
  weight: number
): { pc: number; mode: 'major' | 'minor' } | null {
  ...
  for (let i = 0; i < 12; i++) norm[i] = chroma[i]! / weight / maxC
...
function findBestKeyWithStats(
  ...
  const altModeCorr =
    bestMode === 'major' ? corrMinorAtBestRoot : corrMajorAtBestRoot
  const modeGap = Math.max(0, bestCorr - altModeCorr)

  return { pc: bestPc, mode: bestMode, zScore, bestCorr, secondCorr, modeGap }
}

function computeTonalClarity(
  chroma: Float64Array,
  weight: number,
  best: { pc: number; mode: 'major' | 'minor' }
): number {
