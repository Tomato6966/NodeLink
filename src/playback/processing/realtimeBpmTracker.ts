...
export interface RealtimeBpmTrackerOptions {
  minBpm?: number
  maxBpm?: number
...
const DEFAULTS: Required<RealtimeBpmTrackerOptions> = {
  minBpm: 70,
  maxBpm: 195,
  minIntervalSec: 0.24,
  maxIntervalSec: 1.25,
...
const HARMONIC_CANDIDATES: Array<{ multiplier: number; weight: number }> = [
...
export class RealtimeBpmTracker {
  private readonly opts: Required<RealtimeBpmTrackerOptions>
  private timeSec = 0
  private fastEnv = 0
  private slowEnv = 0
  private fluxMean = 0
  ...
  private lastBeatSec = Number.NEGATIVE_INFINITY
  private intervalsSec: number[] = []
  private bpm = 0
  ...
  private _ingestBeat(beatTimeSec: number, strength: number): void {
    ...
    if (Number.isFinite(this.lastBeatSec)) {
      ...
      if (
        interval >= this.opts.minIntervalSec &&
  ...
  private _normalizeTempo(bpm: number): number {
    if (!Number.isFinite(bpm) || bpm <= 0) return 0
    let v = bpm
    while (v < this.opts.minBpm) v *= 2
    while (v > this.opts.maxBpm) v /= 2
  ...
  private _adaptiveRefractorySec(): number {
    if (!(this.bpm > 0)) {
  ...
  private _normalizeInterval(intervalSec: number): number {
    ...
    if (this.phasePeriodSec > 0) {
      ...
      while (v > target * 1.75) v /= 2
      while (v < target * 0.55) v *= 2
    }
    return v
  }

  private _estimateTempoFromIntervals(): {
    tempo: number
    consistency: number
    separation: number
  } {
  ...
  private _updatePhaseAnchor(beatTimeSec: number): void {
    if (!(this.phasePeriodSec > 0)) {
      this.phaseAnchorSec = beatTimeSec
      return
    }
