...
import type { AudioMixerConfig, MixLayer } from '../../typings/playback/processing.types.ts'
...
export class AudioMixer extends EventEmitter {
     ...
     */
    public mixBuffers(mainPCM: Buffer, layersPCM: Map<string, { buffer: Buffer; volume: number }>): Buffer {
     ...
     */
    public addLayer(stream: VoiceAudioStream, track: PlayerTrack, volume: number | null = null): string {
     ...
     */
    public readLayerChunks(chunkSize: number): Map<string, { buffer: Buffer; volume: number }> {
