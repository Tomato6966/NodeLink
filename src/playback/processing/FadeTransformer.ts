...
import type { FadeCurve, FadeTransformerOptions, IFadeTransformer } from '../../typings/playback/processing.types.ts'
...
const _clamp = (value: number, min: number, max: number): number => Math.max(min, Math.min(max, value))

const _normalizeCurve = (curve: string): FadeCurve =>
    SUPPORTED_CURVES.has(curve as FadeCurve) ? (curve as FadeCurve) : DEFAULT_CURVE
...
export class FadeTransformer extends Transform implements IFadeTransformer {
    ...
    constructor(options: FadeTransformerOptions = {}) {
        ...
        const initialGain = Number.isFinite(options.volume) ? (options.volume as number) : 1.0
     ...
     */
    public fadeTo(volume: number, durationMs: number, curve: string = DEFAULT_CURVE): void {
