...
import type { VolumeTransformerOptions, FadeCurve, IVolumeTransformer } from '../../typings/playback/processing.types.ts'
...
export class VolumeTransformer extends Transform implements IVolumeTransformer {
    ...
    constructor(options: VolumeTransformerOptions = {}) {
        ...
        this.sampleRate = Number.isFinite(sampleRate) && (sampleRate as number) > 0 ? (sampleRate as number) : 48000
        this.channels = Number.isFinite(channels) && (channels as number) >= 1 ? Math.max(1, Math.floor(channels as number)) : 2

        this.lookaheadSamples = Math.max(0, Math.round(((lookaheadMs as number) / 1000) * this.sampleRate)) * this.channels
        ...
        this.fadeDurationMs = Number.isFinite(fadeDurationMs) && (fadeDurationMs as number) >= 0 ? (fadeDurationMs as number) : 1000
        ...
        this.fadeCurve = SUPPORTED_CURVES.has(fadeCurve as FadeCurve) ? (fadeCurve as FadeCurve) : DEFAULT_CURVE

        this.limiterThreshold = Math.min(0.999, Math.max(0, Number.isFinite(limiterThreshold) ? (limiterThreshold as number) : 0.95))
        this.limiterSoftness = Math.max(0.01, Number.isFinite(limiterSoftness) ? (limiterSoftness as number) : 0.4)
    ...
    private _computeFadeGains(sampleCount: number): { gainStart: number; gainEnd: number } {
    ...
    private _prepareView(buffer: Buffer, sampleCount: number): { buffer: Buffer; view: Int16Array } {
    ...
    public override _flush(callback: (error?: Error | null, data?: unknown) => void): void {
        this.push(this.flush())
        callback()
    }

    public override _transform(chunk: Buffer, _encoding: BufferEncoding, callback: (error?: Error | null, data?: unknown) => void): void {
        try {
