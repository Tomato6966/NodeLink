...
export interface AutoMixDecision {
  ...
  /** Blending style for this transition. */
  blendStyle?: 'standard' | 'fusion'
...
export default class AutoMixController {
  ...
  public static analyze(
    ...
    // ── BPM relationship ──
    let bpmMatch: { speedRatio: number; diff: number; variant: string } | null =
      null
    if (
      useBpm &&
    ...
    const getFeatureVector = (t: PlayerTrack, opts: AutoMixAnalyzeOptions) => {
    ...
    // Bayesian selection: The "best" candidates are the ones with lowest cost.
    // We add history penalties directly to the cost function.
    type ScoredCandidate = Candidate & { totalCost: number }

    const tier: ScoredCandidate[] = candidates.slice(0, 4).map((c) => {
    ...
    let pick: ScoredCandidate = tier[0]!
...
function quantizeDurationToPhrase(
  durationMs: number,
  bpm: number,
  minMs: number,
  maxMs: number
...
function readDeezerNumeric(
  track: PlayerTrack,
  field: 'bpm' | 'gain'
): number | null {
  const pluginInfo = (track.pluginInfo || {}) as Record<string, any>
  const deezerMeta =
    pluginInfo['deezer'] || pluginInfo['deezerMetadata'] || null
...
function asValidTempo(value: number | null | undefined): number | null {
  return Number.isFinite(value) && (value as number) > 0 ? Number(value) : null
}
