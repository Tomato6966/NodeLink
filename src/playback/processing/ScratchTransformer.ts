  ...
  private _calculateRate(t: number, state: ScratchState): number {
    ...
    switch (style) {
      ...
      case 'start':
        ...
        if (t < 0.5) return Math.pow(t / 0.5, 2) * 1.5 // Quadratic ramp to 1.5x speed
  ...
  public process(chunk: Buffer): Buffer {
    ...
    for (let f = 0; f < incomingFrames; f++) {
      ...
      if (Math.abs(this.currentRate) < 0.01 && !this.state) {
        for (let c = 0; c < this.channels; c++) outI16[f * this.channels + c] = 0
        continue
      }
      ...
      if (this.inputReadPos >= this.inputWritePos) this.inputReadPos = this.inputWritePos - 1
    }
