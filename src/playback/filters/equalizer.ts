...
export default class Equalizer extends BaseFilter {
    ...
    public override update(settings: FilterSettings): void {
        ...
        for (let i = 0; i < BANDS.length; i++) {
            ...
            if (freq === undefined || gain === undefined || !filterL || !filterR) continue
    ...
    public override process(chunk: Buffer): Buffer {
        for (let i = 0; i < chunk.length; i += 4) {
            ...
            for (let j = 0; j < BANDS.length; j++) {
                ...
                const outL = fl.b0 * left + fl.b1 * fl.x1 + fl.b2 * fl.x2 - fl.a1 * fl.y1 - fl.a2 * fl.y2
                fl.x2 = fl.x1
                fl.x1 = left
                fl.y2 = fl.y1
                fl.y1 = outL
                ...
                const outR = fr.b0 * right + fr.b1 * fr.x1 + fr.b2 * fr.x2 - fr.a1 * fr.y1 - fr.a2 * fr.y2
                fr.x2 = fr.x1
                fr.x1 = right
                fr.y2 = fr.y1
                fr.y1 = outR
