...
async function handler(nodelink, req, res, sendResponse, parsedUrl) {
    if (req.method === 'GET') {
        ...
        if (nodelink.workerManager) {
            currentRefreshToken =
                nodelink.workerManager.liveYoutubeConfig.refreshToken;
            currentVisitorData = nodelink.workerManager.liveYoutubeConfig.visitorData;
            if (!currentRefreshToken)
                currentRefreshToken =
                    nodelink.options.sources.youtube?.clients?.settings?.TV?.refreshToken;
        ...
        const response = {
            refreshToken: currentRefreshToken
                ? maskString(currentRefreshToken, 7)
                : null,
            visitorData: currentVisitorData
                ? maskString(currentVisitorData, 10)
                : null,
    ...
    if (req.method === 'PATCH') {
        ...
        if (!refreshToken && !visitorData) {
            return sendErrorResponse(req, res, 400, 'Bad Request', 'At least one field (refreshToken or visitorData) must be provided.', parsedUrl.pathname);
        }
        if (refreshToken) {
            ...
            try {
                ...
                return sendErrorResponse(req, res, 403, 'Forbidden', `Token validation failed: ${error.message}. No changes were applied.`, parsedUrl.pathname);
            }
        }
        let updatedCount = 0;
        ...
        try {
            if (nodelink.workerManager) {
                ...
                const promises = nodelink.workerManager.workers
                    ...
                    .map((worker) => nodelink.workerManager
                    .execute(worker, 'updateYoutubeConfig', payload)
                    .then(() => 1)
                    .catch((err) => {
            ...
            return sendErrorResponse(req, res, 500, 'Internal Server Error', 'Failed to propagate configuration changes.', parsedUrl.pathname);
        }
    }
}
export default {
