...
export default class AndroidVR extends BaseClient {
    ...
    getClient(context) {
        return {
            client: {
                ...
                userAgent: 'com.google.android.apps.youtube.vr.oculus/1.71.26 (Linux; U; Android 15; eureka-user Build/AP4A.250205.002) gzip',
                deviceMake: 'Google',
                osName: 'Android',
                osVersion: '15',
    ...
    async search(query, _type, context) {
        ...
        try {
            ...
            if (!searchResult) {
                logger('debug', 'YouTube-AndroidVR', `Empty search result for '${query}'.`);
            ...
            if (tracks.length === 0) {
                logger('debug', 'YouTube-AndroidVR', `No processable tracks found on ${sourceName} for: ${query}`);
                return { loadType: 'empty', data: {} };
            }
