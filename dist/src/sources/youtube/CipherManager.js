...
export default class CipherManager {
    ...
    async getPlayerScript() {
        ...
        try {
            if (this.explicitPlayerScriptUrl &&
                ...
                logger('debug', 'YouTube-Cipher', `Using explicit player script URL: ${this.explicitPlayerScriptUrl.url}`);
            ...
            logger('debug', 'YouTube-Cipher', `Obtained player script from watch page: ${this.cachedPlayerScript.url}`);
            return this.cachedPlayerScript;
        }
        finally {
            this.cipherLoadLock = false;
    ...
    async getTimestamp(playerUrl) {
        ...
        if (!this.config.url) {
            const { body: scriptContent, error, statusCode } = await makeRequest(playerUrl, { method: 'GET' });
        ...
        const { body, error, statusCode } = await makeRequest(`${this.config.url}/get_sts`, {
            method: 'POST',
            headers,
            body: requestBody,
    ...
    async checkCipherServerStatus() {
        ...
        try {
            ...
            const { statusCode, error } = await http1makeRequest(`${this.config.url}/`, { method: 'GET', timeout: 5000, headers });
            if (error || statusCode !== 200) {
    ...
    async resolveUrl(streamUrl, encryptedSignature, nParam, signatureKey, playerScript, _context) {
        ...
        const { body, error, statusCode } = await makeRequest(`${this.config.url}/resolve_url`, {
            method: 'POST',
            headers,
            body: requestBody,
            disableBodyCompression: true,
