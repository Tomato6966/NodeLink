...
function extractMetadataFromResponse(fullApiResponse) {
    ...
    if (vd) {
        ...
        if (vd.author && vd.author !== 'undefined')
...
async function fetchOEmbedMetadata(videoId, makeRequest) {
    try {
        const { body, statusCode } = await makeRequest(`https://www.youtube.com/oembed?url=https://www.youtube.com/watch?v=${videoId}&format=json`, {
            method: 'GET',
            timeout: 5000
        });
        ...
        logger('debug', 'fetchOEmbedMetadata', `Failed to fetch oEmbed data: ${e.message}`);
    }
    return null;
}
function extractTitle(renderer, fullApiResponse, videoId, makeRequestFn = null) {
...
function extractAuthor(renderer, fullApiResponse, videoId, makeRequestFn = null) {
    const metadata = extractMetadataFromResponse(fullApiResponse);
    if (metadata.author) {
...
async function fetchChannelInfo(channelId, makeRequest, context) {
    if (!channelId)
        return null;
    try {
        const { body: channelResponse, statusCode } = await makeRequest('https://www.youtube.com/youtubei/v1/browse', {
            method: 'POST',
            headers: {
        ...
        const avatarSources = header.image?.decoratedAvatarViewModel?.avatar?.avatarViewModel?.image
            ?.sources;
        channelInfo.icon =
        ...
        channelInfo.banner =
            Array.isArray(bannerSources) && bannerSources.length > 0
                ...
                : null;
        const accessibilityLabel = header.title?.dynamicTextViewModel?.rendererContext?.accessibilityContext
            ?.label;
        ...
        logger('error', 'fetchChannelInfo', `Failed to fetch channel info: ${e.message}`);
    ...
    if (resolved.spotify &&
        ...
        try {
            ...
            if (response.finalUrl?.includes('spotify.com')) {
                ...
                const match = response.finalUrl.match(/spotify\.com\/(album|track|artist|playlist)\/([a-zA-Z0-9]+)/);
                if (match) {
                    resolved.spotifyId = {
                        type: match[1],
                        id: match[2]
...
function extractExternalLinks(description, _resolve = false, _makeRequest = null) {
    if (!description)
        return null;
    const links = {
        spotify: null,
...
export async function buildTrack(itemData, itemType, sourceNameOverride = null, fullApiResponse = null, enableHolo = false, config = {}) {
    if (!itemData) {
    ...
    let uri = '';
    if (itemType === 'ytmusic') {
        title = safeString(getRunsText(getItemValue(renderer, ['title.runs'])) ||
            getItemValue(renderer, ['title.simpleText']) ||
            renderer?.title, FALLBACK_TITLE);
        if (title === FALLBACK_TITLE && Array.isArray(renderer.flexColumns)) {
            title = safeString(getRunsText(renderer.flexColumns[0]?.musicResponsiveListItemFlexColumnRenderer
                ?.text?.runs), FALLBACK_TITLE);
        }
        ...
        if (author === FALLBACK_AUTHOR && Array.isArray(renderer.flexColumns)) {
            author = safeString(renderer.flexColumns[1]?.musicResponsiveListItemFlexColumnRenderer?.text
                ?.runs?.[0]?.text, FALLBACK_AUTHOR);
        }
        let lengthText = null;
        ...
        const extractedTitle = extractTitle(renderer, fullApiResponse, videoId, makeRequest);
        const extractedAuthor = extractAuthor(renderer, fullApiResponse, videoId, makeRequest);
        if (extractedTitle === null || extractedAuthor === null) {
            try {
                ...
                logger('warn', 'buildTrack', `Failed to fetch oEmbed metadata: ${e.message}`);
                title = FALLBACK_TITLE;
                author = FALLBACK_AUTHOR;
            }
        }
    ...
    if (enableHolo) {
        return await buildHoloTrack(trackInfo, itemData, itemType, fullApiResponse, config);
    }
    return basicTrack;
}
export async function buildHoloTrack(trackInfo, itemData, itemType, fullApiResponse = null, config = {}) {
    ...
    if (config.fetchChannelInfo && channelData.id) {
        try {
            const channelInfo = await fetchChannelInfo(channelData.id, makeRequest, fullApiResponse?.responseContext);
            if (channelInfo) {
            ...
            logger('warn', 'buildHoloTrack', `Failed to fetch channel info: ${e.message}`);
        }
    }
    ...
    if (config.resolveExternalLinks && externalLinks) {
        try {
            ...
            logger('warn', 'buildHoloTrack', `Failed to resolve external links: ${e.message}`);
        }
    }
    ...
    const pluginInfo = {
        ...
        details: {
            ...
            isLive,
            isExplicit: false,
            genres: keywords.length > 0 ? keywords.slice(0, 5) : category ? [category] : [],
            publishedAt
        },
        links: {
...
export async function fetchEncryptedHostFlags(videoId) {
    try {
        ...
        if (error || statusCode !== 200 || !body) {
            logger('warn', 'fetchEncryptedHostFlags', `Failed to fetch embed page: ${statusCode} - ${error?.message}`);
            return null;
        }
        ...
        logger('error', 'fetchEncryptedHostFlags', `Error fetching encryptedHostFlags: ${e.message}`);
        return null;
    }
}
export class BaseClient {
    ...
    async _makePlayerRequest(videoId, context, headers, cipherManager) {
        ...
        if (this.requirePlayerScript() && cipherManager) {
            try {
                ...
                logger('warn', `youtube-${this.name}`, `Failed to get signature timestamp: ${e.message}`);
            }
        }
    ...
    async _handlePlaylistResponse(playlistId, currentVideoId, playlistResponse, sourceName, _context) {
        if (playlistResponse?.error) {
            const errMsg = playlistResponse?.error?.message || 'Failed to fetch playlist.';
        ...
        else if (contentsRoot?.tabbedRenderer?.watchNextTabbedResultsRenderer?.tabs?.[0]
            ?.tabRenderer?.content?.musicQueueRenderer) {
            const musicQueue = contentsRoot.tabbedRenderer.watchNextTabbedResultsRenderer.tabs[0]
                .tabRenderer.content.musicQueueRenderer;
            playlistContent =
    ...
    async _handleBrowsePlaylistResponse(playlistId, browseResponse, sourceName, _context) {
        ...
        const shelf = browseResponse.contents?.singleColumnBrowseResultsRenderer?.tabs?.[0]
        ...
        const playlistTitle = browseResponse.header?.musicDetailHeaderRenderer?.title?.runs?.[0]
            ?.text ||
    ...
    async _extractStreamData(playerResponse, decodedTrack, context, cipherManager, itag) {
        ...
        if (decodedTrack.audioTrackId) {
            const requestedFormats = formats.filter((f) => f.audioTrack && f.audioTrack.id === decodedTrack.audioTrackId);
            if (requestedFormats.length > 0) {
                logger('debug', `youtube-${this.name}`, `Found requested audio track: ${decodedTrack.audioTrackId}`);
                formats = requestedFormats;
            }
            else {
                const hasAudioTracks = formats.some((f) => f.audioTrack);
                if (hasAudioTracks) {
                    logger('warn', `youtube-${this.name}`, `Requested audio track ${decodedTrack.audioTrackId} not found in client ${this.name}.`);
                    return {
                        exception: {
        ...
        const _attemptCipherResolution = async (formatToResolve, playerScript, context) => {
            ...
            if (currentStreamUrl) {
                try {
                    const decipheredUrl = await cipherManager.resolveUrl(currentStreamUrl, currentEncryptedSignature, currentNParam, currentSignatureKey, playerScript, context);
                    formatToResolve.url = decipheredUrl;
                    return formatToResolve;
                }
                catch (e) {
                    logger('warn', `youtube-${this.name}`, `Failed to resolve format URL for itag ${formatToResolve.itag}: ${e.message}`);
                }
            }
            return null;
        };
    ...
    async resolve(url, _type, context, cipherManager) {
        ...
        switch (urlType) {
            ...
            case YOUTUBE_CONSTANTS.SHORTS: {
                ...
                return await this._handlePlayerResponse(playerResponse, sourceName, videoId, context);
            }
    ...
    async getTrackUrl(decodedTrack, context, cipherManager) {
        ...
        return await this._extractStreamData(playerResponse, decodedTrack, context, cipherManager);
    }
}
