...
export default class YandexMusicSource {
    ...
    async setup() {
        const cachedToken = this.nodelink.credentialManager.get('yandexmusic_access_token');
        this.accessToken = this.config.accessToken || cachedToken || null;
        if (!this.accessToken) {
            logger('warn', 'YandexMusic', 'Missing access token. Yandex API disabled; Song.link fallback only.');
            this.hasToken = false;
            return true;
        }
        if (this.config.accessToken && this.config.accessToken !== cachedToken) {
            this.nodelink.credentialManager.set('yandexmusic_access_token', this.config.accessToken, 24 * 60 * 60 * 1000);
        }
        this.hasToken = true;
    ...
    async search(query, sourceTerm, searchType = 'track') {
        ...
        try {
            ...
            const tracks = this._parseTracks(result.tracks?.results || [], 'com').slice(0, limit);
    ...
    async resolve(url) {
        ...
        try {
            ...
            if (match?.groups) {
                return await this._getPlaylist(match.groups.user, match.groups.id, match.groups.domain);
            }
            match = cleanUrl.match(URL_PLAYLIST_UUID_PATTERN);
            if (match?.groups) {
                return await this._getPlaylistByUuid(match.groups.uuid, match.groups.domain);
            }
            return { loadType: 'empty', data: {} };
        }
        catch (e) {
    ...
    _parseTrack(json, domain) {
        if (!json)
            return null;
        if (json.available === false && !this.allowUnavailable)
            return null;
    ...
    _parseCoverUri(json) {
        if (!json)
            return null;
        if (json.ogImage)
            return this._formatCoverUri(json.ogImage);
        if (json.coverUri)
            return this._formatCoverUri(json.coverUri);
    ...
    _buildSongLinkUrl(originalUrl, typeHint, id) {
        if (typeHint === 'album' && id)
            return `https://album.link/ya/${id}`;
        if (typeHint === 'artist' && id)
            return `https://artist.link/ya/${id}`;
        if (typeHint === 'track' && id)
            return `https://song.link/ya/${id}`;
        return originalUrl;
    }
        ...
        try {
            ...
            for (const platform of platforms) {
                ...
                const sourceName = typeof songlinkSource.getPlatformSourceName === 'function'
                    ...
                    : null;
                if (!this._isSourceAvailable(sourceName))
                    continue;
                ...
                if (resolved?.loadType === 'track' ||
                    ...
                    return this._decorateSongLinkResult(resolved, songlinkData, originalUrl, sourceName);
                }
            }
        }
        catch (e) {
    ...
    _decorateSongLinkResult(result, songlinkData, originalUrl, mirrorSource) {
        ...
        const entity = songlinkData?.entitiesByUniqueId?.[songlinkData.entityUniqueId] || null;
    ...
    async _enrichFromSongLinkPlatforms(track, songlinkData) {
        ...
        const needsArtwork = !track.info.artworkUrl || track.info.artworkUrl.includes('yandex.net');
        const needsIsrc = !track.info.isrc;
        if (!needsLength && !needsArtwork && !needsIsrc)
            return;
        const songlinkSource = this.nodelink.sources.getSource('songlink');
        if (!songlinkSource)
            return;
        ...
        for (const platform of platforms) {
            ...
            try {
                ...
                if (doneLength || doneArtwork || doneIsrc)
                    break;
            }
            catch (e) {
                logger('debug', 'YandexMusic', `Song.link metadata resolve failed: ${e.message}`);
            }
        }
    }
    ...
    _applyExternalMetadata(track, info) {
        if (!track.info.title && info.title)
            track.info.title = info.title;
        if (!track.info.author && info.author)
            track.info.author = info.author;
        if (!track.info.length || track.info.length <= 0) {
            if (info.length)
                track.info.length = info.length;
        }
        if (!track.info.artworkUrl ||
            track.info.artworkUrl.includes('yandex.net')) {
            if (info.artworkUrl)
                track.info.artworkUrl = info.artworkUrl;
        }
        if (!track.info.isrc && info.isrc)
            track.info.isrc = info.isrc;
    }
