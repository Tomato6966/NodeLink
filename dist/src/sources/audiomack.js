...
function strictEncodeURIComponent(str) {
    return encodeURIComponent(String(str)).replace(STRICT_URI_RE, (c) => '%' + c.charCodeAt(0).toString(16).toUpperCase());
}
function buildParamString(params) {
    return Object.keys(params)
        .sort()
        .map((k) => `${strictEncodeURIComponent(k)}=${strictEncodeURIComponent(params[k])}`)
        .join('&');
}
...
export default class AudioMackSource {
    ...
    async search(query, _sourceTerm) {
        ...
        try {
            ...
            if (error || !body) {
                logger('error', 'Sources', `[Audiomack] API search failed: ${error?.message}`);
                return {
                    exception: {
            ...
            if (!json) {
                logger('error', 'Sources', '[Audiomack] Invalid JSON in search response.');
    ...
    async getTrackUrl(track) {
        ...
        try {
            ...
            logger('warn', 'Audiomack', `Direct stream failed for ${track.title}: ${e.message}. Falling back to YouTube.`);
        }
        const searchResult = await this.nodelink.sources.searchWithDefault(`${track.title} ${track.author}`);
