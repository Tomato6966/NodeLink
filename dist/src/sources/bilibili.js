    ...
    async resolve(url) {
        ...
        if (videoMatch) {
            ...
            try {
                ...
                if (body.code !== 0) {
                    const errorMsg = body.message === '啥都木有'
        ...
        if (audioMatch) {
            ...
            try {
                if (type === 'au') {
                    const { body } = await makeRequest(`https://www.bilibili.com/audio/music-service-c/web/song/info?sid=${id}`, {
                        method: 'GET',
                        headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                    });
                    if (body.code !== 0)
                    ...
                    const { body } = await makeRequest(`https://www.bilibili.com/audio/music-service-c/web/song/of-menu?sid=${id}&pn=1&ps=100`, {
                        method: 'GET',
                        headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                    });
                    if (body.code !== 0)
                        throw new Error(`Album API Error: ${body.msg}`);
                    ...
                    const { body: infoBody } = await makeRequest(`https://www.bilibili.com/audio/music-service-c/web/menu/info?sid=${id}`, {
                        method: 'GET',
                        headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                    });
                    return {
        ...
        if (liveMatch) {
            ...
            try {
                const { body } = await makeRequest(`https://api.live.bilibili.com/room/v1/Room/get_info?room_id=${id}`, {
                    method: 'GET',
                    headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                });
                if (body.code !== 0)
        ...
        if (spaceMatch) {
            ...
            try {
                ...
                const { body } = await makeRequest(`https://api.bilibili.com/x/space/wbi/arc/search?${query}`, {
                    method: 'GET',
                    headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                });
                if (body.code !== 0)
    ...
    async getTrackUrl(track) {
        try {
            ...
            if (isAudio) {
                ...
                const { body } = await makeRequest(`https://www.bilibili.com/audio/music-service-c/web/url?sid=${sid}`, {
                    method: 'GET',
                    headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                });
            ...
            if (isLive) {
                ...
                const { body } = await makeRequest(`https://api.live.bilibili.com/xlive/web-room/v2/index/getRoomPlayInfo?room_id=${roomId}&protocol=0,1&format=0,2&codec=0,1&qn=10000&platform=web&pt=web&no_playurl=0&mask=0`, {
                    method: 'GET',
                    headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                });
            ...
            if (!cid) {
                const { body } = await makeRequest(`https://api.bilibili.com/x/web-interface/view?bvid=${bvid}`, {
                    method: 'GET',
                    headers: { ...HEADERS, Cookie: this.buildCookieHeader() }
                });
                if (body.code !== 0)
            ...
            const { body } = await makeRequest(`https://api.bilibili.com/x/player/wbi/playurl?${query}`, {
                method: 'GET',
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
                    Referer: 'https://www.bilibili.com/',
