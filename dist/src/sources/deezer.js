...
export default class DeezerSource {
    ...
    async setup() {
        ...
        try {
            ...
            const userDataRes = await makeRequest('https://www.deezer.com/ajax/gw-light.php?method=deezer.getUserData&input=3&api_version=1.0&api_token=', {
                method: 'GET',
                getCookies: true,
                headers: {
            ...
            if (userDataRes.error || !userDataRes.body?.results) {
                throw new Error(`Failed to fetch user data: ${userDataRes.error?.message || 'Invalid response.'}`);
            }
    ...
    async search(query, sourceTerm, searchType = 'track') {
        ...
        const { body, error } = await makeRequest(`https://api.deezer.com/2.0/search/${apiType}?q=${encodeURIComponent(query)}`, { method: 'GET' });
        if (error || body.error) {
            return {
                exception: {
    ...
    async getRecommendations(query) {
        try {
            ...
            const { body: result, error } = await makeRequest(`https://www.deezer.com/ajax/gw-light.php?method=${method}&input=3&api_version=1.0&api_token=${this.csrfToken}`, {
                method: 'POST',
    ...
    async resolve(url) {
        ...
        if (!match)
            return { loadType: 'empty', data: {} };
        const [, type, id] = match;
        logger('debug', 'Sources', `Resolving Deezer URL of type '${type}' with ID '${id}'`);
        ...
        switch (type) {
            ...
            case 'playlist': {
                ...
                for (const item of tracksRes.body.data) {
                    tracks.push(this.buildTrack(item, playlistData.cover_xl || playlistData.picture_xl));
                }
                return {
                    loadType: 'playlist',
                    data: {
            ...
            case 'artist': {
                ...
                const topTracksRes = await makeRequest(`https://api.deezer.com/2.0/artist/${id}/top?limit=${this.config.maxAlbumPlaylistLength || 25}`);
    ...
    buildTrack(item, artworkUrl = null) {
        ...
        if (artistArtworkUrl)
            pluginInfo.artistArtworkUrl = artistArtworkUrl;
        if (previewUrl)
    ...
    async getTrackUrl(decodedTrack, itag, forceRefresh = false) {
        ...
        if (this.licenseToken) {
            try {
                const { body: trackData } = await makeRequest(`https://www.deezer.com/ajax/gw-light.php?method=song.getListData&input=3&api_version=1.0&api_token=${this.csrfToken}`, {
                    method: 'POST',
                    headers: { Cookie: this.cookie },
    ...
    loadStream(decodedTrack, url, _format, additionalData) {
        return new Promise(async (resolve) => {
            try {
                ...
                let i = 0;
                const headers = {};
                if (additionalData.startTime !== undefined &&
                ...
                if (res.error || (res.statusCode !== 200 && res.statusCode !== 206)) {
                    ...
                    logger('error', 'Sources', `Error fetching Deezer stream: ${error.message}`);
                    return resolve({
                        exception: {
                ...
                logger('error', 'Sources', `Failed to load Deezer stream for ${decodedTrack.identifier}: ${e.message}`);
