    ...
    async resolve(url) {
        try {
            ...
            const extractArtworkFromImgAlt = () => {
                ...
                if (altIdx === -1)
                    altIdx = html.indexOf('alt="song thumbnail"');
                if (altIdx === -1)
                    return null;
            ...
            const extractIsrcFromHtml = () => {
                ...
                for (let t = 0; t < tokens.length; t++) {
                    ...
                    while (true) {
                        ...
                        while (html.charCodeAt(i) === 92)
                            i++;
                        if (html.charCodeAt(i) !== 34)
                            continue;
                        i++;
                        if (i + 12 > html.length)
                            continue;
                        if (!isUpper(html.charCodeAt(i)) ||
                            !isUpper(html.charCodeAt(i + 1)))
                            continue;
                        if (!isUpperOrDigit(html.charCodeAt(i + 2)) ||
            ...
            const appleMusicUrl = extractHrefStartingAt('href="https://www.shazam.com/applemusic/song/');
            const durationMs = (() => {
                const findIso = () => {
                    const needles = [
                        '"duration":"PT',
