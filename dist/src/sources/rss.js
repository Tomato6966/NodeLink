...
export default class RssSource {
    ...
    async resolve(url) {
        try {
            ...
            const channelTitle = this._extractTagText(channelXml, 'title') || 'RSS Feed';
            const channelImage = this._extractTagAttr(channelXml, 'itunes:image', 'href') ||
                this._extractTagText(this._extractFirstTag(channelXml, 'image') || '', 'url') ||
                null;
            ...
            for (const itemXml of items) {
                ...
                const author = this._extractTagText(itemXml, 'itunes:author') ||
                ...
                const artwork = this._extractTagAttr(itemXml, 'itunes:image', 'href') ||
                    channelImage ||
                    null;
                if (!enclosureUrl)
                    continue;
    ...
    _extractTagAttr(xml, tag, attr) {
        const tagRe = new RegExp(`<${this._escape(tag)}\\b[^>]*>`, 'i');
        const match = tagRe.exec(xml);
        if (!match)
            return null;
    ...
    _buildTagRegex(tag) {
        return new RegExp(`<${this._escape(tag)}\\b[^>]*>([\\s\\S]*?)<\\/${this._escape(tag)}>`, 'i');
    }
