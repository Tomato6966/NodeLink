...
export default class IheartradioSource {
    ...
    async _getStreamUrl(stationId) {
        try {
            const data = await this._request(`${IHEART_API_V2}/content/liveStations/${stationId}`);
            if (!data)
                return null;
            const station = data.hits?.[0] || data.station || data;
            const streams = station.streams || {};
            const streamUrl = streams.shoutcast_stream ||
    ...
    async search(query, _sourceTerm) {
        try {
            ...
            const data = await this._request(`${IHEART_API_V1}/catalog/searchAll` +
                `?keywords=${encoded}` +
                `&bestMatch=True` +
                `&queryStation=True` +
                `&queryArtist=False` +
    ...
    async loadStream(decodedTrack, url) {
        try {
            const streamUrl = url || (await this._getStreamUrl(decodedTrack.identifier));
            if (!streamUrl) {
                throw new Error(`Could not resolve stream URL for iHeart station ${decodedTrack.identifier}.`);
            }
