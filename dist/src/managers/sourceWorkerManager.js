...
const resolveSourceExecPath = () => {
    ...
    if (fs.existsSync(distSourceWorker))
        return distSourceWorker;
...
const buildSourceWorkerExecArgv = () => {
    ...
    const maxOldSpaceMb = Number(process.env.NODELINK_SOURCE_WORKER_MAX_OLD_SPACE_MB || 0);
...
class SourceWorkerManager {
    constructor(nodelink) {
        ...
        this.maxWorkers = Math.max(1, nodelink.options.cluster?.specializedSourceWorker?.count || 1);
    ...
    _forkWorker() {
        ...
        worker.on('message', (msg) => {
            if (msg.type === 'ready')
                logger('info', 'SourceCluster', `Source worker manager ${msg.pid} ready`);
        });
        worker.on('error', (err) => {
            logger('error', 'SourceCluster', `Source worker ${worker.id} error: ${err.message}`);
        });
    ...
    async executeAll(task, payload, options = {}) {
        ...
        const settled = await Promise.allSettled(targets.map((worker) => this._executeOnWorker(worker, task, payload, options).then((response) => ({
            clusterId: worker.id,
