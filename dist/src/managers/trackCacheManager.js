...
export default class TrackCacheManager {
    ...
    async load() {
        try {
            ...
            if (code !== 'ENOENT') {
                logger('error', 'TrackCache', `Failed to load track cache: ${getErrorMessage(error)}`);
            }
            this.cache = new Map();
        }
    }
    ...
    async forceSave() {
        try {
            ...
            logger('error', 'TrackCache', `Failed to save track cache: ${getErrorMessage(error)}`);
        }
    }
    /**
