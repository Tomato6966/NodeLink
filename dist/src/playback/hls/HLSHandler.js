...
export default class HLSHandler extends PassThrough {
    ...
    isLive;
    playlistTimer;
    activeSegmentStreams;
    lastMediaSequence;
    highestSequence;
    stuckCount;
    preRolled;
    justResynced;
    masterRefreshCounter;
    /**
    ...
    _rememberSegment(key) {
        if (this.processedSegments.has(key))
            return false;
    ...
    async _playlistLoop() {
        ...
        try {
            ...
            let parsed;
            try {
                parsed = parsePlaylist(playlistContent, this.currentUrl);
            }
            catch (e) {
                if (this.currentUrl !== this.masterUrl) {
                    this.currentUrl = this.masterUrl;
            ...
            await this._handleMediaPlaylist(parsed, playlistContent);
        }
        catch (err) {
            const error = err;
            if (!this.isLive) {
    ...
    /** @internal */
    async _handleMasterPlaylist(parsed) {
        if (!parsed.isMaster)
            return;
        const sortedVariants = parsed.variants.sort((a, b) => b.bandwidth - a.bandwidth);
        const bestVariant = sortedVariants.find((v) => (v.codecs?.includes('mp4a') || v.codecs?.includes('opus')) &&
            !v.codecs?.includes('avc1')) ??
            sortedVariants.find((v) => v.codecs?.includes('mp4a') || v.codecs?.includes('opus')) ??
            sortedVariants[0];
        if (!bestVariant) {
    ...
    /** @internal */
    async _handleMediaPlaylist(parsed, playlistContent) {
        ...
        const newSegments = parsed.segments.filter((s) => {
            if (s.sequence !== -1 && s.sequence <= this.highestSequence)
                return false;
    ...
    async _fetchSegments() {
        if (this.isFetching || this.stop)
            return;
        this.isFetching = true;
        const fetchPool = new Map();
        const fillPool = () => {
            while (fetchPool.size < this.maxParallelFetches &&
        ...
        while ((this.segmentQueue.length > 0 || fetchPool.size > 0) && !this.stop) {
            ...
            if (this.isLive &&
                ...
                if (this.segmentQueue.length === 0 && fetchPool.size === 0)
                    break;
                continue;
            }
            if (fetchPool.size === 0)
            ...
            if (!nextEntry.value)
                continue;
            const [key, promise] = nextEntry.value;
