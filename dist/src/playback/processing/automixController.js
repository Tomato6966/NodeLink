...
import { camelotDistance, harmonicCompatibilityScore, harmonicPitchShift } from "./keyDetector.js";
export default class AutoMixController {
     ...
     */
    static _computeOptimalDuration(maxDurationMs, bpmA, bpmB, keyDist, trackBEnergy, trackALengthMs, trackBLengthMs) {
        ...
        if (bpm && bpm > 0) {
            ...
            if (bpm >= 118 && bpm <= 132)
                genre = 'house';
            else if (bpm >= 115 && bpm <= 145)
                genre = 'electronic';
            else if (bpm >= 80 && bpm <= 110)
                genre = 'hiphop';
            else if (bpm >= 110 && bpm <= 170)
                genre = 'rock';
        ...
        const floorMs = Math.min(maxDurationMs, maxDurationMs >= 6000
            ? 6000
    ...
    static analyze(trackA, trackB, crossfadeDurationMs, mode, options = {}) {
        ...
        crossfadeDurationMs = AutoMixController._computeOptimalDuration(configuredDurationMs, bpmA, bpmB, -1 /* keyDist computed below */, trackBEnergy, trackA.info.length || 0, trackB.info.length || 0);
        ...
        if (keyDist >= 0) {
            crossfadeDurationMs = AutoMixController._computeOptimalDuration(configuredDurationMs, bpmA, bpmB, keyDist, trackBEnergy, trackA.info.length || 0, trackB.info.length || 0);
        }
        if (harmonicScore != null) {
            if (harmonicScore >= 0.84) {
                crossfadeDurationMs = Math.min(configuredDurationMs, Math.round(crossfadeDurationMs * 1.08));
            }
            else if (harmonicScore <= 0.46) {
                crossfadeDurationMs = Math.max(4500, Math.round(crossfadeDurationMs * 0.92));
            }
        }
        ...
        if (strongEnergyRiseEarly || energeticOpeningJumpEarly) {
            ...
            crossfadeDurationMs = Math.max(floorMs, Math.min(configuredDurationMs, stretched));
            if ((bpmA ?? bpmB) && (bpmA ?? bpmB) > 0) {
                crossfadeDurationMs = quantizeDurationToPhrase(crossfadeDurationMs, (bpmA ?? bpmB), floorMs, configuredDurationMs);
            }
        }
        ...
        const bpmConfidence = bpmMatch
            ...
            : 0;
        const lowConfidenceTempo = !!(bpmMatch &&
        ...
        if (incomingGainMultiplier == null &&
            isEnergyKnown &&
            trackAEnergy > 0 &&
            trackBEnergy > 0) {
        ...
        if (decision.transition === 'fusion_morph' ||
            decision.transition === 'harmonic_weave') {
            const fusionFloorMs = mode === 'fusion' ? 16000 : 14000;
            decision.transitionDurationMs = Math.max(decision.transitionDurationMs, fusionFloorMs);
        }
        ...
        if (decision.transition === 'filter_sweep' &&
            bpmMatch &&
            bpmMatch.diff >= 0.24 &&
            trackBEnergy >= 0 &&
            trackBEnergy < 0.12) {
        ...
        const fusionTempoTransition = decision.transition === 'fusion_morph' ||
            decision.transition === 'harmonic_weave';
        ...
        if (tempoMatch && bpmMatch && bpmMatch.diff <= tempoMatchTolerance && allowTempoTimescale) {
            ...
            const targetSpeed = Math.max(1 - adaptiveSpeedClamp, Math.min(1 + adaptiveSpeedClamp, rawSpeed));
    ...
    static _applySignaturePolish(decision, context) {
        ...
        if (!polishedTransitions.has(decision.transition))
            return;
        if (contrast < 0.45 && !context.energyPivotMode)
            return;
        ...
        const fusionLike = decision.transition === 'fusion_morph' ||
            decision.transition === 'harmonic_weave';
        if (fusionLike) {
        ...
        if (context.energyPivotMode) {
            decision.incomingPanCompletionRatio = Math.max(decision.incomingPanCompletionRatio ?? 0.28, 0.45);
            decision.incomingEchoCompletionRatio = Math.max(decision.incomingEchoCompletionRatio ?? 0.64, 0.76);
        }
    }
}
// ── Transition Builders ──
...
function readDeezerNumeric(track, field) {
    ...
    if (!deezerMeta || typeof deezerMeta !== 'object')
        return null;
...
function selectTempoCandidate(input) {
    ...
    logger('debug', 'AutoMix', `Tempo source fallback to metadata for Track ${input.trackLabel} (live conf ${(liveConf ?? 0).toFixed(2)}, diff ${(diff * 100).toFixed(1)}%)`);
    return { bpm: meta, source: metaSource ?? 'metadata' };
}
