...
export default class AutoMixController {
   ...
   */
  static _computeOptimalDuration(
    maxDurationMs,
    bpmA,
    bpmB,
    keyDist,
    ...
    if (keyDist >= 0) {
      ...
      else if (keyDist > 2) duration *= 0.9
    }
    ...
    const clamped = Math.round(
      Math.max(floorMs, Math.min(maxDurationMs, duration))
    )
    if (bpm && bpm > 0) {
   ...
   */
  static analyze(trackA, trackB, crossfadeDurationMs, mode, options = {}) {
    ...
    if (trackA.info.isStream || trackB.info.isStream) {
      logger(
        'info',
        'AutoMix',
        `Stream detected → gapless: [${titleA}] → [${titleB}]`
      )
      return gapless()
    }
    // ── Same album → gapless ──
    const piA = trackA.pluginInfo
    const piB = trackB.pluginInfo
    ...
    if (albumA && albumA === albumB) {
      logger(
        'info',
        'AutoMix',
        `Same album → gapless: [${titleA}] → [${titleB}]`
      )
      return gapless()
    }
    ...
    const maxBpmDiffRatio =
      typeof deezerCfg.maxBpmDiffRatio === 'number' &&
    ...
    let incomingGainMultiplier = null
    ...
    const strongEnergyRiseEarly =
      energyKnownEarly &&
      trackAEnergy > 0 &&
      trackAEnergy < 0.16 &&
      trackBEnergy > 0.18 &&
      trackBEnergy > trackAEnergy * 1.55
    const energeticOpeningJumpEarly =
      energyKnownEarly &&
      trackAEnergy > 0 &&
      trackBEnergy > 0.24 &&
    ...
    logger(
      'info',
      'AutoMix',
      `Optimal duration: ${crossfadeDurationMs}ms` +
    ...
    const bpmConfidence = bpmMatch
      ...
      : 0
    const _lowConfidenceTempo = !!(
      bpmMatch &&
      bpmMatch.diff <= maxBpmDiffRatio &&
      bpmConfidence < 0.78
    )
    const _lowConfidenceVariantTempo = !!(
      bpmMatch &&
      bpmMatch.diff <= maxBpmDiffRatio &&
    ...
    const _durationRatio =
      durationA > 0 && durationB > 0
        ? Math.abs(durationA - durationB) / Math.max(durationA, durationB)
        : 0
    const _avgDuration = ((durationA || 180_000) + (durationB || 180_000)) / 2
    const _isShort = durationA < 30_000 || durationB < 30_000
    const _hasTempoData = !!bpmMatch
    const _hasKeyData = keyDist >= 0
    const isEnergyKnown = trackAEnergy >= 0 && trackBEnergy >= 0
    const _highEnergyPair =
      isEnergyKnown && trackAEnergy > 0.35 && trackBEnergy > 0.16
    const strongEnergyRise =
      isEnergyKnown &&
      trackAEnergy > 0 &&
      trackAEnergy < 0.16 &&
      trackBEnergy > 0.18 &&
      trackBEnergy > trackAEnergy * 1.55
    const energeticOpeningJump =
      isEnergyKnown &&
      trackAEnergy > 0 &&
      trackBEnergy > 0.24 &&
      trackBEnergy - trackAEnergy > 0.1
    ...
    const strongEnergyDrop =
      isEnergyKnown && trackAEnergy > 0.25 && trackBEnergy < 0.12
    const _largeGainCorrection =
    ...
    const getFeatureVector = (t, _opts) => {
    ...
    const euclideanDist = Math.sqrt(
      vA.reduce((sum, val, i) => sum + (val - (vB[i] || 0)) ** 2, 0)
    )
    ...
    const tier = candidates.slice(0, 4).map((c) => {
      ...
      if (recency === 0) cost += 0.5
      else if (recency === 1) cost += 0.25
    ...
    if (harmonicMixScore >= 0.85 && isCloseBpm) {
      const fusionCandidate = candidates.find(
        (c) => c.transition === 'fusion_morph'
      )
      if (fusionCandidate) {
        logger(
          'info',
          'AutoMix',
          `Best Fusion match detected (harm: ${Math.round(harmonicMixScore * 100)}%, bpm-diff: ${(bpmMatch.diff * 100).toFixed(1)}%) -> forcing fusion_morph`
        )
    ...
    if (energyPivotMode && trackBEnergy >= 0.22) {
      ...
      const pivotPick = candidates.find((c) =>
        pivotTransitions.has(c.transition)
      )
    ...
    if (
      ...
      if (energyPivotMode) {
        incomingGainMultiplier = Math.max(
          ...
          Math.min(
            0.8,
            incomingGainMultiplier != null ? incomingGainMultiplier : 0.72
          )
        )
      }
    }
    ...
    if (
      ...
      if (Math.abs(targetSpeed - 1.0) >= 0.015) {
        decision.timescaleA = {
          ...
          durationMs: Math.max(
            2500,
            Math.min(15000, Math.round(decision.transitionDurationMs * 1.2))
          ),
          curve: 'sinusoidal'
        }
        if (
    ...
    const harmonicTag =
      harmonicScore != null ? ` harm:${Math.round(harmonicScore * 100)}%` : ''
    const bpmTag = bpmMatch
      ? ` bpm-diff:${(bpmMatch.diff * 100).toFixed(1)}%(${bpmMatch.variant}, conf:${Math.round(bpmConfidence * 100)}%)`
      : ''
    const energyTag =
      trackBEnergy >= 0 ? ` energy:${(trackBEnergy * 100).toFixed(0)}%` : ''
    logger(
      'info',
      'AutoMix',
      `${decision.transition} (fitness cost ${pick.totalCost.toFixed(2)}) for guild: ` +
        `[${titleA}] → [${titleB}]${keyTag}${harmonicTag}${bpmTag}${energyTag} | ${topScores}`
    )
    return decision
  }
  static _applySignaturePolish(decision, context) {
    ...
    const keyContrast =
      context.keyDist >= 0
        ...
        : 0.35
    const tempoContrast =
      context.bpmDiff != null
        ? Math.max(0, Math.min(1, context.bpmDiff / 0.3))
        : 0.25
    const energyContrast =
      context.trackAEnergy >= 0 && context.trackBEnergy >= 0
        ? Math.max(
            0,
            Math.min(
              1,
              Math.abs(context.trackAEnergy - context.trackBEnergy) / 0.35
            )
          )
        : 0.3
    ...
    if (fusionLike) {
      ...
      decision.highpassSweepAlpha = Math.max(
        0.04,
        Math.min(0.1, hpBase + hpBoost)
      )
      decision.lowpassSweepAlpha = Math.max(
        0.07,
        Math.min(0.12, lpBase + lpBoost)
      )
      decision.lowpassSweepCompletionRatio = context.strongEnergyDrop
        ? 0.62
        : 0.52
    ...
    if (context.energyPivotMode) {
      if (fusionLike) {
        ...
        decision.lowpassSweepAlpha = Math.max(
          decision.lowpassSweepAlpha ?? 0.1,
          0.105
        )
        decision.lowpassSweepCompletionRatio = Math.max(
    ...
    if (
      ...
      decision.echoA.mix = Math.max(
        0.08,
        Math.min(0.14, decision.echoA.mix - 0.02)
      )
    }
    // If contrast is extreme and energy drops hard, avoid too much wetness.
    if (context.strongEnergyDrop && context.largeBpmGap) {
      if (decision.reverbA) {
        decision.reverbA.mix = Math.max(
          0.1,
          Math.min(0.24, decision.reverbA.mix)
        )
      }
      if (decision.echoA) {
...
function keySignalReliability(key) {
  if (!key) return 0
 ...
 */
function crossfadeEq(durationMs, withEcho = true, bpm = null) {
...
function readDeezerNumeric(track, field) {
...
function readDeezerText(track, field) {
  const pluginInfo = track.pluginInfo || {}
  const deezerMeta = pluginInfo.deezer || pluginInfo.deezerMetadata || null
  if (!deezerMeta || typeof deezerMeta !== 'object') return null
...
function tempoSourceConfidence(source) {
  ...
  if (source.includes('audio')) return 0.8
  if (
    source.includes('deezer') ||
    source.includes('api') ||
...
function computeTempoConfidence(sourceA, sourceB, variant) {
  let confidence =
    (tempoSourceConfidence(sourceA) + tempoSourceConfidence(sourceB)) / 2
  if (variant !== 'direct') confidence *= 0.7
...
function selectTempoCandidate(input) {
  ...
  if (live && !meta) {
    return {
      bpm: live,
      source:
        liveConf != null && liveConf < 0.4 ? 'audio-live-lowconf' : 'audio-live'
    }
  }
  if (!live && meta) {
 ...
 */
function highpassDissolve(durationMs, bpm = null) {
  const ramp = Math.round(durationMs * 0.85)
 ...
 */
function cinemaLift(durationMs, bpm = null) {
 ...
 */
function filterSweep(durationMs, bpm = null) {
  const ramp = Math.round(durationMs * 0.85)
 ...
 */
function reverbWash(durationMs, bpm = null) {
 ...
 */
function beatSyncedDelay(bpm, subdivision, fallback) {
