...
export class CrossfadeController extends Transform {
    ...
    _applyAdaptiveBandUnmasking(mainL, mainR, nextL, nextR, progress, transientNorm, energyNorm) {
        ...
        const drive = Math.max(0, Math.min(1, transientNorm * 0.72 + energyNorm * 0.45));
        ...
        const lowMask = Math.max(0, Math.min(1, Math.min(mainLowAbs, nextLowAbs) / 14000)) *
            overlap;
        let incomingLowDuck = lowMask * (0.05 + early * 0.14 + drive * 0.07);
        if (fusionActive)
            incomingLowDuck *= 1.08;
        incomingLowDuck *=
        ...
        const airMask = Math.max(0, Math.min(1, Math.min(mainAirAbs, nextAirAbs) / 9000)) *
            overlap;
        ...
        const sidechainWindow = this._smoothStep01((progress - 0.12) / 0.70) *
            (1 - this._smoothStep01((progress - 0.86) / 0.14));
        let sidechain = sidechainWindow * (0.02 + drive * 0.08 + lowMask * 0.04);
        if (fusionActive)
            sidechain *= 1.05;
                    ...
                    if (this._bcfRuntime && !this._bcfRuntime.isFinished) {
                        ...
                        if (trackCChunk) {
                            ...
                            for (let f = 0; f < totalFrames; f++) {
                                ...
                                let cR = this.channels > 1 ? paddedC.readInt16LE((base + 1) * 2) : cL;
                                if (this._hpEnabled && progress < 1) {
                                    const hpP = Math.min(1, progress / 0.3);
