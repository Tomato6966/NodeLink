...
export class RealtimeBpmTracker {
    ...
    push(onset, deltaSec) {
        ...
        const beatDetected = flux > threshold && rising && beatGap >= refractory && strength > 0;
    ...
    _ingestBeat(beatTimeSec, strength) {
        ...
        if (Number.isFinite(this.lastBeatSec)) {
            ...
            if (interval >= this.opts.minIntervalSec &&
                ...
                const rawConfidence = RealtimeBpmTracker._clamp(consistency * 0.48 +
                    separation * 0.26 +
                    sampleFactor * 0.16 +
                    strengthFactor * 0.1, 0, 1);
    ...
    _updatePhaseAnchor(beatTimeSec) {
        ...
        if (Math.abs(this.timeSec - this.phaseAnchorSec) >
