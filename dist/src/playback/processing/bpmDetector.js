 ...
 */
export function estimateBpmFromPcm(pcm, sampleRate, channels) {
    const WINDOW_MS = 10;
    ...
    if (numWindows < Math.round(3000 / WINDOW_MS))
        return null;
    ...
    for (let i = meanWin; i < onset.length; i++) {
        let sum = 0;
        for (let j = i - meanWin; j < i; j++)
            sum += onset[j];
        onset[i] = Math.max(0, onset[i] - sum / meanWin);
    }
 ...
 */
export function estimateBpmFromOnsets(onsets, onsetsPerSecond) {
    // Need at least 4 seconds
    if (onsets.length < Math.round(onsetsPerSecond * 4))
        return null;
...
function autocorrelateBpm(onset, samplesPerSecond) {
    ...
    if (maxCorr < 1e-10)
        return null; // No rhythmic content
    // Collect local peaks that exceed 30 % of the maximum
    const peaks = [];
    for (let lag = minLag + 1; lag < maxLag; lag++) {
        if (corr[lag] > corr[lag - 1] &&
            corr[lag] > corr[lag + 1] &&
            corr[lag] > maxCorr * 0.3) {
            peaks.push({ lag, strength: corr[lag] });
        }
    }
    let bestLag;
    if (peaks.length === 0) {
    ...
    if (halfLag >= minLag) {
        ...
        if (halfStrength > corr[bestLag] * 0.65) {
    ...
    if (doubleLag <= maxLag) {
        ...
        if (doubleStrength > corr[bestLag] * 0.75 && bestBpm > 140) {
