...
export class DASHHandler extends Transform {
    ...
    async start() {
        ...
        try {
            ...
            if (mpdRes.error ||
                ...
                const err = new Error(`MPD fetch failed: ${mpdRes.error || mpdRes.statusCode}`);
            ...
            if (initRes.error ||
                ...
                const err = new Error(`Init segment fetch failed: ${initRes.error || initRes.statusCode}`);
                logger('error', 'DASHHandler', err.message);
                this.destroy(err);
                return;
            }
            ...
            if (this.options.startTime &&
                ...
                segmentDuration > 0) {
                skipSegments = Math.floor(this.options.startTime / (segmentDuration * 1000));
                if (skipSegments >= totalSegments)
                    skipSegments = totalSegments - 1;
                if (skipSegments < 0)
                    skipSegments = 0;
            }
            logger('debug', 'DASHHandler', `Total segments: ${totalSegments}, prefetch: ${PREFETCH_COUNT}, skip: ${skipSegments}`);
            const fetchQueue = [];
            let fetchIndex = 0;
            let pushIndex = 0;
            ...
            while (fetchQueue.length < PREFETCH_COUNT && fetchIndex < totalSegments) {
                ...
                if (url)
                    fetchQueue.push(fetchSegment(url));
                fetchIndex++;
            }
            ...
            logger('debug', 'DASHHandler', `All ${pushIndex} segments pushed (skipped ${skipSegments}), ending stream`);
            this.push(null);
            this.emit('finishBuffering');
        }
        catch (err) {
            if (!signal.aborted) {
                logger('error', 'DASHHandler', `Stream error: ${err.message}`);
                this.destroy(err);
            }
        }
    }
    ...
    *_generateSegmentUrls(selected) {
    ...
    _calcSegmentDuration(selected) {
        if (selected.segments.length === 0)
            return 0;
        const total = selected.segments.reduce((sum, seg) => sum + seg.duration * (seg.repeat + 1), 0);
     ...
     */
    _parseRepresentations(mpdContent) {
        ...
        while (repMatch !== null) {
            ...
            const audioSamplingRate = parseInt(urlAttr(repAttrs, 'audioSamplingRate') || '44100', 10);
            ...
            const startNumber = parseInt(urlAttr(templateAttrs, 'startNumber') || '1', 10);
            const timescale = parseInt(urlAttr(templateAttrs, 'timescale') || '44100', 10);
            const segments = [];
            const sRegex = /<S\b([^>]*)\/?>/g;
            let sMatch;
     ...
     */
    _selectRepresentation(representations) {
        let best = null;
