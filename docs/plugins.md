...
Where Plugins Live
- Local (recommended for development): place files in `src/plugins/*.js`.
  - Example: `src/plugins/example-plugin.js` is included and registers a `/v4/example-plugin/ping` route.
...
- `api.registerStreamInterceptor(fn)`
  ...
  - Signature: `(nodelink, track, url, protocol, additionalData, next) => Readable|Promise<Readable>`
  - Call `await next()` to get the original stream, then return your wrapped stream.
...
Audio Cache Example
```js
// src/plugins/audio-cache-plugin.js
...
export default async function (nodelink, api) {
  api.registerStreamInterceptor(async (server, track, url, protocol, additionalData, next) => {
    ...
    // Return original or a PassThrough tee’d into a file
    return original
  })
}
```
...
```js
// src/plugins/audio-normalizer-plugin.js
export default async function (nodelink, api) {
